...
export async function POST(req) {
    try {
        ...
        const insertQuery = `
            INSERT INTO five_grams_link_ayat (five_gram_id, ayat_id) 
            VALUES ($1, $2)
        ...
        try {
            ...
            for (const row of records) {
                const [fiveGramId, ayatId] = [row[0], row[3]]; // Extract required fields
                try {
                    await client.query(insertQuery, [fiveGramId, ayatId]);
                    console.log(`Inserted record: FiveGramID=${fiveGramId}, AyatID=${ayatId}`);
                } catch (error) {{
        ...
        return NextResponse.json({ message: "File uploaded and five-gram-links inserted successfully!" });
