...
export async function POST(req) {
    try {
        ...
        const insertQuery = `
            INSERT INTO bi_grams_link_ayat (bi_gram_id, ayat_id) 
            VALUES ($1, $2)
        ...
        try {
            ...
            for (const row of records) {
                const [biGramId, ayatId] = [row[0], row[2]]; // Extract required fields
                try {
                    await client.query(insertQuery, [biGramId, ayatId]);
                    console.log(`Inserted record: BiGramID=${biGramId}, AyatID=${ayatId}`);
                } catch (error) {{
