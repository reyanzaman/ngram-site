...
export async function POST(req) {
    try {
        ...
        const insertQuery = `
            INSERT INTO tri_grams (id, tri_gram_text, bi_gram_id) 
            VALUES ($1, $2, $3)
        ...
        try {
            ...
            for (const row of records) {
                const [triGramId, triGramText, biGramId] = [row[0], row[1], row[3]];
                try {
                    await client.query(insertQuery, [triGramId, triGramText, biGramId]);
                    console.log(`Inserted record: ID=${triGramId}, Tri-gram=${triGramText}, Bi-gram ID=${biGramId}`);
                } catch (error) {
                    if (error.code === "23505") {
                        console.warn(`Conflict: Tri-gram ID ${triGramId} already exists.`);
                    } else {
                        throw error;
                    }
                }
        ...
        return NextResponse.json({ message: "File uploaded and tri-grams inserted successfully!" });
