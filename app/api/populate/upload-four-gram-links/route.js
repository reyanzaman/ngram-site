...
export async function POST(req) {
    try {
        ...
        const insertQuery = `
            INSERT INTO four_grams_link_ayat (four_gram_id, ayat_id) 
            VALUES ($1, $2)
        ...
        try {
            ...
            for (const row of records) {
                const [fourGramId, ayatId] = [row[0], row[3]]; // Extract required fields
                try {
                    await client.query(insertQuery, [fourGramId, ayatId]);
                    console.log(`Inserted record: FourGramID=${fourGramId}, AyatID=${ayatId}`);
                } catch (error) {{
        ...
        return NextResponse.json({ message: "File uploaded and four-gram-links inserted successfully!" });
