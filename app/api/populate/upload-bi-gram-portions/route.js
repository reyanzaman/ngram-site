...
export async function POST(req) {
    try {
        ...
        const insertQuery = `
            INSERT INTO bi_grams_portions (bi_gram_id, ayat_id, left_portion, right_portion) 
            VALUES ($1, $2, $3, $4)
        ...
        try {
            ...
            for (const row of records) {
                const [biGramId, ayatId, leftPortionRaw, rightPortionRaw] = [row[0], row[2], row[3], row[4]]; // Extract required fields
                const leftPortion = leftPortionRaw === "0" ? null : leftPortionRaw;
                const rightPortion = rightPortionRaw === "0" ? null : rightPortionRaw;
                
                try {
                    await client.query(insertQuery, [biGramId, ayatId, leftPortion, rightPortion]);
                    console.log(`Inserted record: BiGramID=${biGramId}, AyatID=${ayatId}, leftPortion=${leftPortion}, rightPortion=${rightPortion}`);
                } catch (error) {{
                    console.warn(`Error: Bi-gram ID ${biGramId} and Ayat ID ${ayatId} already exists`);
                    throw error;
                }}
            }
        ...
        return NextResponse.json({ message: "File uploaded and bi-gram-portions inserted successfully!" });
