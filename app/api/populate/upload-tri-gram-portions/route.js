...
export async function POST(req) {
    try {
        ...
        const insertQuery = `
            INSERT INTO tri_grams_portions (tri_gram_id, ayat_id, left_portion, right_portion) 
            VALUES ($1, $2, $3, $4)
        ...
        try {
            ...
            for (const row of records) {
                const [triGramId, ayatId, leftPortionRaw, rightPortionRaw] = [row[0], row[3], row[4], row[5]]; // Extract required fields
                const leftPortion = leftPortionRaw === "0" ? null : leftPortionRaw;
                const rightPortion = rightPortionRaw === "0" ? null : rightPortionRaw;
                
                try {
                    await client.query(insertQuery, [triGramId, ayatId, leftPortion, rightPortion]);
                    console.log(`Inserted record: TriGramID=${triGramId}, AyatID=${ayatId}, leftPortion=${leftPortion}, rightPortion=${rightPortion}`);
                } catch (error) {{
        ...
        return NextResponse.json({ message: "File uploaded and Tri-gram-portions inserted successfully!" });
