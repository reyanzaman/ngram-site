...
export async function POST(req) {
    try {
        ...
        const insertQuery = `
            INSERT INTO four_grams_portions (four_gram_id, ayat_id, left_portion, right_portion) 
            VALUES ($1, $2, $3, $4)
        ...
        try {
            ...
            for (const row of records) {
                const [fourGramId, ayatId, leftPortionRaw, rightPortionRaw] = [row[0], row[3], row[4], row[5]]; // Extract required fields
                const leftPortion = leftPortionRaw === "0" ? null : leftPortionRaw;
                const rightPortion = rightPortionRaw === "0" ? null : rightPortionRaw;
                
                try {
                    await client.query(insertQuery, [fourGramId, ayatId, leftPortion, rightPortion]);
                    console.log(`Inserted record: FourGramID=${fourGramId}, AyatID=${ayatId}, leftPortion=${leftPortion}, rightPortion=${rightPortion}`);
                } catch (error) {{
                    console.warn(`Error: Four-gram ID ${fourGramId} and Ayat ID ${ayatId} already exists`);
                    throw error;
                }}
            }
        ...
        return NextResponse.json({ message: "File uploaded and Four-gram-portions inserted successfully!" });
