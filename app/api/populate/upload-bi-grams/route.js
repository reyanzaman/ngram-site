...
export async function POST(req) {
    try {
        ...
        const insertQuery = `
            INSERT INTO bi_grams (id, bi_gram_text, topic_id) 
            VALUES ($1, $2, $3)
        ...
        try {
            ...
            for (const row of records) {
                const [biGramId, biGramText, topicId] = row; // Extract required fields
                try {
                    await client.query(insertQuery, [biGramId, biGramText, topicId]);
                    console.log(`Inserted record: ID=${biGramId}, Bi-gram=${biGramText}, Topic ID=${topicId}`);
                } catch (error) {
                    if (error.code === "23505") {
                        console.warn(`Conflict: Bi-gram ID ${biGramId} already exists.`);
                    } else {
                        throw error;
                    }
                }
        ...
        return NextResponse.json({ message: "File uploaded and bi-grams inserted successfully!" });
